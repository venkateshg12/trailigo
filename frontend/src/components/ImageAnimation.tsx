...
const ImageAnimation = () => {
    ...
    const animateSlideIn = (el: HTMLElement | null) => {
        if (!el) return;
        gsap.fromTo(el, { opacity: 0, scale: 1.1 }, { opacity: 1, scale: 1, duration: 1, ease: "power3.out", });
    };
    ...
    useGSAP(() => {
        ...
        if (addressRef.current) {
            gsap.fromTo(addressRef.current, { opacity: 0 }, { opacity: 1, y: 0, duration: 1.8, ease: "power2.out", });
        }
    }, [currentIndex]);
